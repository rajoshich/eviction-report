...
number_evictions <- nrow(evictions)
...
by_month_plot <- ggplot(by_month) + 
  ...
  labs(
    title = "Number of evictions/month in San Francisco", 
...
evictions_plot <- base_plot +
  geom_point(mapping = aes(x = long, y = lat), color = "blue", alpha = 0.3) +
  labs(title = "Evictions in San Francisco in 2017") +
